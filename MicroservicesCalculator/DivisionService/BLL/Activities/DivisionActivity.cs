...
public class DivisionActivity(ISender sender) : IActivity<OperationArguments, OperationCalculationLog>
{
    public async Task<ExecutionResult> Execute(ExecuteContext<OperationArguments> context)
    {
        var command = new CalculateDivisionOperationCommand(context.Arguments.ResultOperand, context.Arguments.CalculationOperand);
