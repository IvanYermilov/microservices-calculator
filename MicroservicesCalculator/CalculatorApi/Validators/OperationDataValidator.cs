...
public class OperationDataValidator : AbstractValidator<OperationData>
{
    public OperationDataValidator()
    {
        ...
        RuleFor(data => data.Expression).MaximumLength(50).WithMessage("Expression you input has length more then 50 characters");
        ...
        ).WithMessage("Expression should contain mathematical operations('+', '-', '*', '/'), numbers and symbol '.'(dot) only");
    }
}